...
subroutine Scalar_ord4_calc_rhs( CCTK_ARGUMENTS )
  ...
  CCTK_REAL                Tmatt(4,4), trTm, matter_dens   !Energy momentum tensor associated to matter and its trace
  ...
  CCTK_INT                 a, b, c, m
  CCTK_REAL, parameter ::  pi = 4.d0*atan(1.0)
  ...
  !$OMP PRIVATE(alph, beta, ch, hh, hu, trk, dethh, TrTm, Tmatt,&
 ...
 !   Tmatt(1,1) = mTxx(i,j,k)
 !   Tmatt(1,2) = mTxy(i,j,k)
 !   Tmatt(1,3) = mTxz(i,j,k)
 !   Tmatt(2,2) = mTyy(i,j,k)
 !   Tmatt(2,3) = mTyz(i,j,k)
 !   Tmatt(3,3) = mTzz(i,j,k)
 !   Tmatt(4,1) = mTtx(i,j,k)
 !   Tmatt(4,2) = mTty(i,j,k)
 !   Tmatt(4,3) = mTtz(i,j,k)
 !   Tmatt(4,4) = mTtt(i,j,k)
 !   Tmatt(3,4) = Tmatt(4,3)
 !   Tmatt(2,4) = Tmatt(4,2)    
 !   Tmatt(1,4) = Tmatt(4,1)    
 !   Tmatt(3,2) = Tmatt(2,3)    
 ...
 !   Tmatt(2,1) = Tmatt(1,2)    
    ...
    !----------- Compute trace of the energy momentum tensor associated to matter as T= S-rho ---------

     
    matter_dens = Tmatt(4,4)/(alph*alph) !--- rho*(1+e) in the non-rotating case
    trTm = -matter_dens 

     do a = 1,3
        trTm = trTm + 2*beta(a)*Tmatt(a,4)/(alph*alph)
        do b = 1,3 
            trTm = trTm + (ch*hu(a,b)-beta(a)*beta(b)/(alph*alph)) * Tmatt(a,b)
        end do
      end do 
    ...
    do a = 1, 3
      do b = 1, 3
        ...
        !-----modify according to eq. 4.23 from arxiv.1505.07462-----
    ...
    rhs_lKphi1 = rhs_lKphi1 - 0.5d0 * ch * tr_dalp_dphi1                                                      &
                 + 0.5d0 * alph * ( - ch * tr_cd2_phi1 + 0.5d0 * tr_dch_dphi1                                 &
                                    ...
                                    * ( lphi1*lphi1 + lphi2*lphi2 ) * ( lphi1*lphi1 + lphi2*lphi2 ) )         &
                                    + 2 * trk * lKphi1 )                                                      &
                 + 0.5d0 * ext_force1 * lphi1 - 2 * pi * alph * ( betaDEF * lphi1 + k0BD) * TrTm
  ...
  if ( ( check_rho == 1 ) .and. ( use_jacobian .eqv. .false. ) ) then
    matter_density(i,j,k) = 0.1 !matter_dens
    end if
    !-------------------------------------------
