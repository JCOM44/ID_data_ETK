...
subroutine Scalar_calc_Tmunu( CCTK_ARGUMENTS )
  ...
  do k = 1+cctk_nghostzones(3), cctk_lsh(3)-cctk_nghostzones(3)
     do j = 1+cctk_nghostzones(2), cctk_lsh(2)-cctk_nghostzones(2)
        do i = 1+cctk_nghostzones(1), cctk_lsh(1)-cctk_nghostzones(1)
           ...
           eTtt(i,j,k) =  Tab(4,4) + mTtt(i,j,k)
           eTtx(i,j,k) =  Tab(4,1) + mTtx(i,j,k)
           eTty(i,j,k) =  Tab(4,2) + mTty(i,j,k)
           eTtz(i,j,k) =  Tab(4,3) + mTtz(i,j,k)
           eTxx(i,j,k) =  Tab(1,1) + mTxx(i,j,k)
           eTxy(i,j,k) =  Tab(1,2) + mTxy(i,j,k)
           eTxz(i,j,k) =  Tab(1,3) + mTxz(i,j,k)
           eTyy(i,j,k) =  Tab(2,2) + mTyy(i,j,k)
           eTyz(i,j,k) =  Tab(2,3) + mTyz(i,j,k)
           eTzz(i,j,k) =  Tab(3,3) + mTzz(i,j,k)
  ...
  !  matter_dens = Tmatt(4,4)/(alph*alph*(1+eps(i,j,k)))

                !------------ Write to grid functions ---------------------
                rhoSF_gf(i,j,k)    = rhoSF
                jrSF_gf(i,j,k)     = jrSF
                SrrSF_gf(i,j,k)    = SrrSF
   !             matter_density(i,j,k) = matter_dens
                !----------------------------------------------------------
