...
subroutine JordanFBSSN_calc_bssn_rhs( CCTK_ARGUMENTS )
  ...
  CCTK_REAL                d1_lphi(3),d1_lKphi(3), ad1_lphi, ad1_lKphi, D_phi(4)
  CCTK_REAL                d2_lphi(3,3),cd2_lphi(3,3)
  CCTK_REAL                tr_dalp_dphi, tr_cd2_phi, tr_dww_dphi
  CCTK_REAL                src_trT, coupling_term, T_scalar(4,4)
  ...
  evolve_alp  = CCTK_EQUALS(lapse_evolution_method, "JordanBSSN")
  evolve_beta = CCTK_EQUALS(shift_evolution_method, "JordanBSSN")
  ...
  !$OMP lphi,lKphi,d1_lphi,d1_lKphi, ad1_lphi,ad1_lKphi, &
  !$OMP d2_lphi, cd2_lphi, src_trT, D_phi,  &
  !$OMP tr_dalp_dphi, tr_cd2_phi, tr_dww_dphi, &
  !$OMP coupling_term, T_scalar, &
  ...
  if (use_jacobian) then
    ...
    !Calculate scalar stress-energy tensor
...
!----------------- Start decoupling
      ...
      if (CCTK_EQUALS(theory,"decEF")) then
Tab = 0

       Tab(4,4) = eTtt(i,j,k) +  sfTtt(i,j,k)
       Tab(4,1) = eTtx(i,j,k) +  sfTtx(i,j,k)
       Tab(4,2) = eTty(i,j,k) +  sfTty(i,j,k)
       Tab(4,3) = eTtz(i,j,k) +  sfTtz(i,j,k)
       Tab(1,1) = eTxx(i,j,k) +  sfTxx(i,j,k)
       Tab(1,2) = eTxy(i,j,k) +  sfTxy(i,j,k)
       Tab(1,3) = eTxz(i,j,k) +  sfTxz(i,j,k)
       Tab(2,2) = eTyy(i,j,k) +  sfTyy(i,j,k)
       Tab(2,3) = eTyz(i,j,k) +  sfTyz(i,j,k)
       Tab(3,3) = eTzz(i,j,k) +  sfTzz(i,j,k)
       Tab(1,4) = Tab(4,1)
       Tab(2,4) = Tab(4,2)
       Tab(3,4) = Tab(4,3)
       Tab(2,1) = Tab(1,2)
...
!----------------- Start decoupling
    ...
    !--- Coupling term in the Kphi eq.
        coupling_term = - 2 * pi * alph * ( 0.01) * src_trT

    !------ 
    rhs_lKphi = rhs_lKphi - 0.5d0 * ww*ww * tr_dalp_dphi    &
   + 0.5d0 * alph * ( - ww*ww * tr_cd2_phi +  ww * tr_dww_dphi             &
       + 2 * trk * lKphi ) + coupling_term

  !-------------------------------------------
         if (r(i,j,k)==0.0) THEN
  write(*,*) "-----------------------------------"
  write(*,*) "The value of rhs_phi at r=0 is", rhs_lphi
  write(*,*) "The value of rhs_kphi at r=0 is", rhs_lKphi
  write(*,*) "The coupling term is ", coupling_term
  write(*,*) "tr_dphi_dalph  ", tr_dalp_dphi
  write(*,*) "tr_cd2phi ", tr_cd2_phi

  write(*,*) "tr_dphi_dww  ", tr_dww_dphi
  write(*,*) "d2_phi ", d2_lphi
  write(*,*) "d1_phi ", d1_lphi
...
subroutine JordanFBSSN_calc_bssn_rhs_bdry( CCTK_ARGUMENTS )
  ...
  if (CCTK_EQUALS(lapse_evolution_method, "JordanBSSN")) then
     ierr = NewRad_Apply(cctkGH, alp, rhs_alp, one, one, n_alpha)
  end if

  if (CCTK_EQUALS(shift_evolution_method, "JordanBSSN")) then
