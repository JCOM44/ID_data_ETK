...
subroutine JordanFBSSN_bssn_constraints( CCTK_ARGUMENTS )
  ...
  CCTK_REAL                cd2_ww(3,3), cd1_aa(3,3,3), cd2_phi(3,3)
  ...
  CCTK_REAL                tr_cd2_Bphi, tr_dBphi_dBphi, tr_dphi_dphi
  CCTK_REAL                phi_2, phi_4, Bphi, BKphi
  CCTK_REAL                omega_phi, d_omega_phi
  CCTK_REAL                d1_Bphi(3), d1_Bkphi(3)
  CCTK_REAL                cd2_Bphi(3,3) 
  
  ! Auxiliar variables
  CCTK_REAL                covD_phi(3,3)
  CCTK_REAL                omega_mom(3),Bphi_mom(3)  
  CCTK_REAL                omega_ham, Bphi_ham, aux
  ...
  !$OMP d1_ww, d1_hh, d1_trk, d1_aa, d1_gammat, d1_phi, d1_kphi, &
  !$OMP d2_ww, d2_hh, d2_phi, cd2_phi, cd2_ww, cd1_aa, ham, mom, &
  !$OMP cf1, cf2, c_ri, c_ri_ww, c_ri_hh, ri_1,                  &
  !$OMP ri_2, ri_3, sq_aa, a2, trr,                              &
  !$OMP phi_2, phi_4, Bphi, Bkphi, omega_phi, d_omega_phi,       &
  !$OMP d1_Bphi, d1_Bkphi, cd2_Bphi,                              &
  !$OMP covD_phi, aux, omega_mom, Bphi_mom, omega_ham, Bphi_ham, & 
  !$OMP tr_cd2_ww, tr_dww_dww, tr_cd2_Bphi, tr_dBphi_dBphi,  tr_dphi_dphi, &
  !$OMP srcE, srcS, srcjdi,                                      &
  ...
  do i = 1+cctk_nghostzones(1), cctk_lsh(1)-cctk_nghostzones(1)
    ...
    do a = 1, 3
      do b = a, 3
        do l = 1, 3
          ...
          cd2_phi(a,b) = cd2_phi(a,b) - cf2(l,a,b) * d1_phi(l)
        do m = 1, 3
    ...
    do a = 1, 3
      do b = 1, 3
        ...
        aux       = aux + hu(a,b) * d1_phi(a) * d1_ww(b)
      end do
    end do
    covD_phi = ww * ( covD_phi - hh * aux ) + ww*ww * cd2_phi
    !-------------------------------------------
    ...
    ! cd2_Bphi   = Bphi * (1 + varphi*varphi) * d1_varphi * d1_varphi + varphi * Bphi * cd2_phi
...
!-------------- BD
    if(CCTK_EQUALS(theory,"BD")) then   ! Start defining BD
    Bphi         = exp(phi_sf)
    BKphi        = Bphi * kphi_sf
...
!-------------------DEF
    ...
    ! Because of the structure of the eqs, we decide to calculate tr_cd2_phi and tr_dphi_dphi here
    ...
    do l = 1, 3
      do m = 1, 3
        ...
        tr_dBphi_dBphi = tr_dBphi_dBphi + hu(l,m) * d1_Bphi(l)*d1_Bphi(m) * ww*ww 
        tr_dphi_dphi = tr_dphi_dphi  +  hu(l,m) * d1_phi(l)*d1_phi(m) * ww*ww 
      end do
    end do
    ! Note: we are implementing W^2 R_{ij} here
    ...
    !-------Scalar field constraint terms-------------------
    
    omega_ham = omega_phi/(Bphi*Bphi) * (Bkphi*Bkphi + tr_dBphi_dBphi)
    Bphi_ham  = 2.0d0/Bphi * ( tr_cd2_Bphi -  trk*Bkphi )
    ...
    ham = trr + 2.0d0 * trk**2 / 3.0d0 - sq_aa
    if(CCTK_EQUALS(theory,"DEF") .OR. CCTK_EQUALS(theory,"BD")) then
            ham = ham - omega_ham - Bphi_ham
    end if 
    !--- Momentum constraint 
    mom = -2.0d0 * d1_trk / 3.0d0
    do a = 1, 3
    if(CCTK_EQUALS(theory,"DEF") .OR. CCTK_EQUALS(theory,"BD")) then
      !-- Add terms that go as omega(phi) 
        omega_mom(a) = omega_phi/(Bphi*Bphi) * Bkphi*d1_Bphi(a)
        mom(a) = mom(a) - omega_mom(a)
      !-- Add terms that go as 1/Phi 
        Bphi_mom(a)  = 1.0d0/Bphi * (d1_Bkphi(a) - 1.0d0/3.0d0 * trk * d1_Bphi(a))
      end if 
      do l = 1, 3
        do m = 1, 3
    if(CCTK_EQUALS(theory,"DEF") .OR. CCTK_EQUALS(theory,"BD")) then
          !-- Calculate term h^{ab} A_{i,a} D_{b} Phi 
          Bphi_mom(a) = Bphi_mom(a) - 1.0d0/Bphi * (hu(l,m) * aa(a,l) * d1_Bphi(m))
  else 
          Bphi_mom(a) = 0.0
        end if
          !-- Add the last term to the momentum constraint and calculate the GR terms 
          mom(a) = mom(a) + hu(l,m) * ( cd1_aa(a,l,m) - 3.0d0 * aa(a,l) * d1_ww(m) / ww ) - Bphi_mom(a)
        end do
      end do
    end do
