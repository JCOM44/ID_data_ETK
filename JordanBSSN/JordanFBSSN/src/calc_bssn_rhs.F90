...
subroutine JordanFBSSN_calc_bssn_rhs( CCTK_ARGUMENTS )
  ...
  CCTK_REAL                phi_sf, kphi_sf

  ! Scalar field variables
  CCTK_REAL                tr_cd2_Bphi, tr_dBphi_dBphi, tr_dww_dphi
  CCTK_REAL                tr_cd2_phi, tr_dphi_dphi, tr_dphi_dalph
  CCTK_REAL                phi_2, phi_4, Bphi, BKphi
  CCTK_REAL                omega_phi, d_omega_phi, coupling_term
  CCTK_REAL                d1_Bphi(3), d1_BKphi(3)
  CCTK_REAL                cd2_Bphi(3,3) 
  CCTK_REAL                K_omega, K_Bphi, F_phi
  CCTK_REAL                aa_omega(3,3), aa_Bphi(3,3)
  CCTK_REAL                gammat_omega(3), gammat_Bphi(3) 
  ...
  CCTK_REAL                ad1_phi, ad1_kphi

  ! Covaraint derivatives
  CCTK_REAL                cd2_ww(3,3), cd2_alph(3,3), cd2_phi(3,3)
  ...
  CCTK_REAL                tr_cd2_ww, tr_dww_dww, aux, aux1
  CCTK_REAL                divbeta, aadbeta(3,3), hhdbeta(3,3)
  CCTK_REAL                covD_phi(3,3)
  ...
  CCTK_REAL                rhs_ww, rhs_hh(3,3), rhs_trk, rhs_aa(3,3),      &
                           rhs_gammat(3), rhs_beta(3), rhs_vphi, rhs_vkphi
  ...
  evolve_alp  = CCTK_EQUALS(lapse_evolution_method, "JordanFBSSN")
  evolve_beta = CCTK_EQUALS(shift_evolution_method, "JordanFBSSN")
  ...
  !$OMP d1_beta1, d1_beta2, d1_beta3, d1_phi, d1_kphi,        &
  ...
  !$OMP d2_ww, d2_hh, d2_alph, d2_beta, d2_phi, cd2_phi, &
  !$OMP d1_f, cf1, cf2, ad1_phi, ad1_kphi, &
  !$OMP ad1_ww, ad1_hh, ad1_trk, ad1_aa, ad1_gammat, ad1_alph, ad1_beta, &
  !$OMP cd2_ww, cd2_alph, ri_1, ri_2, ri_3, c_ri, c_ri_ww, c_ri_hh, &
  !$OMP tr_cd2_ww, tr_dww_dww, tr_dphi_dalph, tr_dww_dphi, &
  !$OMP c_ll, aux, f_shift, divbeta, gamcon, &
  !$OMP rhs_ww, rhs_hh, rhs_trk, rhs_aa, rhs_gammat, rhs_beta, rhs_vphi, rhs_vkphi,&
  !$OMP hhdbeta, aadbeta, tr_ll, sq_aa, a2, trr, tf_c_ll, tf_c_ri, au,  &
  !$OMP myeta, r2, phi_sf, kphi_sf, F_phi, coupling_term,&
  !$OMP phi_2, phi_4, Bphi, BKphi, omega_phi, d_omega_phi,       &
  !$OMP d1_Bphi, d1_BKphi, cd2_Bphi, tr_dphi_dphi, tr_cd2_phi,    &
  !$OMP K_omega, K_Bphi, aa_omega, aa_Bphi, gammat_omega, gammat_Bphi, &
  !$OMP covD_phi, aux1, tr_cd2_Bphi, tr_dBphi_dBphi, & 
  ...
  do i = 1+cctk_nghostzones(1), cctk_lsh(1)-cctk_nghostzones(1)
    ...
    if (derivs_order == 6) then
      ...
      d1_phi(1) = (  phi1(i+3,j,k) - 9*phi1(i+2,j,k) + 45*phi1(i+1,j,k)          &
                  - phi1(i-3,j,k) + 9*phi1(i-2,j,k) - 45*phi1(i-1,j,k) ) * odx60

      d1_phi(2) = (  phi1(i,j+3,k) - 9*phi1(i,j+2,k) + 45*phi1(i,j+1,k)          &
                  - phi1(i,j-3,k) + 9*phi1(i,j-2,k) - 45*phi1(i,j-1,k) ) * ody60

      d1_phi(3) = (  phi1(i,j,k+3) - 9*phi1(i,j,k+2) + 45*phi1(i,j,k+1)          &
                  - phi1(i,j,k-3) + 9*phi1(i,j,k-2) - 45*phi1(i,j,k-1) ) * odz60

        ! d1_kphi(3)

      d1_kphi(1) = (  Kphi1(i+3,j,k) - 9*Kphi1(i+2,j,k) + 45*Kphi1(i+1,j,k)          &
                  - Kphi1(i-3,j,k) + 9*Kphi1(i-2,j,k) - 45*Kphi1(i-1,j,k) ) * odx60

      d1_kphi(2) = (  Kphi1(i,j+3,k) - 9*Kphi1(i,j+2,k) + 45*Kphi1(i,j+1,k)          &
                  - Kphi1(i,j-3,k) + 9*Kphi1(i,j-2,k) - 45*Kphi1(i,j-1,k) ) * ody60

      d1_kphi(3) = ( Kphi1(i,j,k+3) - 9*Kphi1(i,j,k+2) + 45*Kphi1(i,j,k+1)          &
      ...
      ! d2_phi(3,3)
      d2_phi(1,1) = (  2*phi1(i+3,j,k) - 27*phi1(i+2,j,k) + 270*phi1(i+1,j,k) - 490*phi1(i,j,k)&
                    + 2*phi1(i-3,j,k) - 27*phi1(i-2,j,k) + 270*phi1(i-1,j,k) ) * odxsq180

      d2_phi(2,2) = (  2*phi1(i,j+3,k) - 27*phi1(i,j+2,k) + 270*phi1(i,j+1,k) - 490*phi1(i,j,k)&
                    + 2*phi1(i,j-3,k) - 27*phi1(i,j-2,k) + 270*phi1(i,j-1,k) ) * odysq180

      d2_phi(3,3) = (  2*phi1(i,j,k+3) - 27*phi1(i,j,k+2) + 270*phi1(i,j,k+1) - 490*phi1(i,j,k)&
                    + 2*phi1(i,j,k-3) - 27*phi1(i,j,k-2) + 270*phi1(i,j,k-1) ) * odzsq180

      d2_phi(1,2) = (    -phi1(i-3,j+3,k) +   9*phi1(i-2,j+3,k) -   45*phi1(i-1,j+3,k) +   45*phi1(i+1,j+3,k) -   9*phi1(i+2,j+3,k) +    phi1(i+3,j+3,k) &
      ...
      d2_phi(1,3) = (    -phi1(i-3,j,k+3) +   9*phi1(i-2,j,k+3) -   45*phi1(i-1,j,k+3) +   45*phi1(i+1,j,k+3) -   9*phi1(i+2,j,k+3) +    phi1(i+3,j,k+3) &
      ...
      d2_phi(2,3) = (    -phi1(i,j-3,k+3) +   9*phi1(i,j-2,k+3) -   45*phi1(i,j-1,k+3) +   45*phi1(i,j+1,k+3) -   9*phi1(i,j+2,k+3) +    phi1(i,j+3,k+3) &
      ...
      d2_phi(2,1) = d2_phi(1,2)
      d2_phi(3,1) = d2_phi(1,3)
      d2_phi(3,2) = d2_phi(2,3)
      ...
      if( use_advection_stencils /= 0 ) then
        ...
        ad1_phi  = beta_l(1)*d1_f(1) + beta_l(2)*d1_f(2) + beta_l(3)*d1_f(3)
        ...
        ad1_kphi  = beta_l(1)*d1_f(1) + beta_l(2)*d1_f(2) + beta_l(3)*d1_f(3)
        ...
        ! ad1_phi
        ad1_phi = beta_l(1)*d1_phi(1) + beta_l(2)*d1_phi(2) + beta_l(3)*d1_phi(3)

        !ad1_kphi
        ad1_kphi = beta_l(1)*d1_kphi(1) + beta_l(2)*d1_kphi(2) + beta_l(3)*d1_kphi(3)
  ...
  ! d1_phi(3)
    d1_phi(1) = (   -phi1(i+2,j,k) + 8*phi1(i+1,j,k)                          &
                   - 8*phi1(i-1,j,k) +   phi1(i-2,j,k) ) / dx12

    d1_phi(2) = (   -phi1(i,j+2,k) + 8*phi1(i,j+1,k)                          &
                   - 8*phi1(i,j-1,k) +   phi1(i,j-2,k) ) / dy12

    d1_phi(3) = (   -phi1(i,j,k+2) + 8*phi1(i,j,k+1)                          &
                   - 8*phi1(i,j,k-1) +   phi1(i,j,k-2) ) / dz12

               ! d1_kphi(3)
    d1_kphi(1) = (   -Kphi1(i+2,j,k) + 8*Kphi1(i+1,j,k)                          &
                    - 8*Kphi1(i-1,j,k) +   Kphi1(i-2,j,k) ) / dx12

    d1_kphi(2) = (   -Kphi1(i,j+2,k) + 8*Kphi1(i,j+1,k)                          &
                    - 8*Kphi1(i,j-1,k) +   Kphi1(i,j-2,k) ) / dy12

    d1_kphi(3) = (   -Kphi1(i,j,k+2) + 8*Kphi1(i,j,k+1)                          &
      ...
      ! d2_phi(3,3)
    d2_phi(1,1) = (   -phi1(i+2,j,k) + 16*phi1(i+1,j,k) - 30*phi1(i,j,k)     &
                    + 16*phi1(i-1,j,k) -    phi1(i-2,j,k) ) / dxsq12

    d2_phi(2,2) = (   -phi1(i,j+2,k) + 16*phi1(i,j+1,k) - 30*phi1(i,j,k)     &
                    + 16*phi1(i,j-1,k) -    phi1(i,j-2,k) ) / dysq12

    d2_phi(3,3) = (   -phi1(i,j,k+2) + 16*phi1(i,j,k+1) - 30*phi1(i,j,k)     &
                    + 16*phi1(i,j,k-1) -    phi1(i,j,k-2) ) / dzsq12

    d2_phi(1,2) = (   -phi1(i-2,j+2,k) +  8*phi1(i-1,j+2,k) -  8*phi1(i+1,j+2,k) +   phi1(i+2,j+2,k)   &
    ...
    d2_phi(1,3) = (   -phi1(i-2,j,k+2) +  8*phi1(i-1,j,k+2) -  8*phi1(i+1,j,k+2) +   phi1(i+2,j,k+2)   &
    ...
    d2_phi(2,3) = (   -phi1(i,j-2,k+2) +  8*phi1(i,j-1,k+2) -  8*phi1(i,j+1,k+2) +   phi1(i,j+2,k+2)   &
  ...
  if (use_jacobian) then
    ...
    call JordanFBSSN_apply_jacobian(d1_kphi, jac)
    call JordanFBSSN_apply_jacobian(d1_phi, jac)
    ...
    call JordanFBSSN_apply_jacobian2(d1_alph, d2_alph, jac, hes)
    call JordanFBSSN_apply_jacobian2(d1_ww, d2_ww, jac, hes)
    call JordanFBSSN_apply_jacobian2(d1_phi, d2_phi, jac, hes)
    call JordanFBSSN_apply_jacobian2(d1_beta1, d2_beta1, jac, hes)
    call JordanFBSSN_apply_jacobian2(d1_beta2, d2_beta2, jac, hes)
    call JordanFBSSN_apply_jacobian2(d1_beta3, d2_beta3, jac, hes)
    ...
    do a = 1, 3
      do b = a, 3
        do m = 1, 3
          ...
          cd2_phi(a,b)  = cd2_phi(a,b) - cf2(m,a,b) * d1_phi(m)
    ...
    cd2_phi(2,1)  = cd2_phi(1,2)
    cd2_phi(3,1)  = cd2_phi(1,3)
    cd2_phi(3,2)  = cd2_phi(2,3)
    ...
    !------------ DD_phi ---------------------
    ! Note: we implement W^2 D_{a}D_{b}\phi = W^2 covD Dphi - C Dphi

    aux1 = 0
    do a = 1, 3
      do b = 1, 3
        covD_phi(a,b) = d1_phi(a) * d1_ww(b) + d1_phi(b) * d1_ww(a) 
        aux1          = aux1 + hu(a,b) * d1_phi(a) * d1_ww(b)
      end do
    end do

     covD_phi = ww* (covD_phi - aux1*hh ) + ww*ww*cd2_phi
    ...
    !
    ! For decoupling we should get an equivalence with the
    ! eqs in Einstein frame, which means the eqs for GR with
    ! additional sources. Omega should go to infinity, but numerically
    ! we just set it as a large enough value.
    
    phi_2        = phi_sf * phi_sf
    phi_4        = phi_2 * phi_2

!------------decoupliong
    if(CCTK_EQUALS(theory,"decoupling") .OR. CCTK_EQUALS(theory,"GR")) then
    Bphi        = 1.0
    BKphi       = 0.0
    omega_phi   = 10.0e8
    d_omega_phi = 0.0 
...
!-------------- BD
    if(CCTK_EQUALS(theory,"BD")) then   ! Start defining BD. here omega is a constant?
    Bphi         = exp(phi_sf)
    BKphi        = Bphi * kphi_sf
    omega_phi    = 1.0/(2.0d0*C_BD)-3.0d0/2.0d0 ! this is alpha_0**2 or k_0 **2 
    d_omega_phi  = 0.0
    do a = 1,3
        d1_Bphi(a)    = Bphi *  d1_phi(a)
        d1_BKphi(a)   = Bphi * kphi_sf * d1_phi(a) +  Bphi * d1_kphi(a)
        
        do b = 1,3
        ! We implement W^2 D_{a}D_{b}\Bphi, remember that covD_phi is multiplied already by W^2 in the definition
                cd2_Bphi(a,b)  = ww*ww*( Bphi  * d1_phi(a) * d1_phi(b)) +  Bphi * covD_phi(a,b)
         end do
     end do 
    end if                              ! End of BD definitions

!--------------------- DEF

    if(CCTK_EQUALS(theory,"DEF")) then  ! Start defining DEF
    Bphi         = exp(phi_2/2.0d0)
    BKphi        = Bphi * phi_sf * kphi_sf
    omega_phi    = 2.0d0/(B_DEF*phi_2)-3.0d0/2.0d0
    d_omega_phi  = -4.0d0/(B_DEF * Bphi * phi_4)
    do a = 1,3
        d1_Bphi(a)    = Bphi * phi_sf * d1_phi(a)
        d1_BKphi(a)   = Bphi * kphi_sf * (1.0d0 + phi_2) * d1_phi(a) + phi_sf * Bphi * d1_kphi(a)
        
        do b = 1,3
        ! We implement W^2 D_{a}D_{b}\Bphi, remember that covD_phi is multiplied already by W^2 in the definition
                cd2_Bphi(a,b)  = ww*ww*( Bphi * (1.0d0 + phi_2) * d1_phi(a) * d1_phi(b)) + phi_sf * Bphi * covD_phi(a,b)
         end do
     end do 
   end if                       ! End of DEF definitions
    !------------------------------------------------------- 

    !---- Calculating useful traces and contractions

    ! Note: We calculate here the trace terms 
    ...
    do l = 1, 3
      do m = 1, 3
        ...
        !----W^2 h^{ab} D_{a}D_{b} \Bphi
        tr_cd2_Bphi   = tr_cd2_Bphi + hu(l,m) * cd2_Bphi(l,m) 
        if(CCTK_EQUALS(theory,"decoupling")) then 
                tr_cd2_phi    = tr_cd2_phi + hu(l,m) * cd2_phi(l,m)
        else
                tr_cd2_phi   = tr_cd2_phi + hu(l,m) * covD_phi(l,m) !cd2_phi(l,m) * ww*ww
        end if
        tr_dww_dww  = tr_dww_dww + hu(l,m) * d1_ww(l) * d1_ww(m)
        tr_dww_dphi = tr_dww_dww + hu(l,m) * d1_ww(l) * d1_phi(m)
        !----W^2 h^{ab} D_{a}\Bphi D_{b}\Bphi
        tr_dBphi_dBphi = tr_dBphi_dBphi  +  hu(l,m) * d1_Bphi(l)*d1_Bphi(m)  
        tr_dphi_dphi = tr_dphi_dphi  +  hu(l,m) * d1_phi(l)*d1_phi(m)  
        tr_dphi_dalph = tr_dphi_dalph  +  hu(l,m) * d1_alph(l)*d1_phi(m) 
        ...
        tr_dphi_dalph = tr_dphi_dalph * ww*ww 
    !------------ Ricci Tensor -----------------
    ...
    if (stress_energy_state /= 0) then
       ...
       srcE = srcE / (alph * alph)
       ! Adding source rho_phi for decoupling in E-frame
       if(CCTK_EQUALS(theory,"decoupling")) then
               srcE = srcE + 1.0d0/pi8 *tr_dphi_dphi * kphi_sf*kphi_sf
       end if
       ...
       ! Adding source j_phi for decoupling in E-frame
       if(CCTK_EQUALS(theory,"decoupling")) then
                srcjdi=srcjdi + 1.0d0/pi4 * d1_phi * kphi_sf
       end if
       ...
       do a = 1, 3
          do b = 1, 3
             srcSij(a,b) = Tab(a,b)
       ! Adding source Sij_phi for decoupling in E-frame
             if(CCTK_EQUALS(theory,"decoupling")) then
                srcSij(a,b) = srcSij(a,b) + 1.0d0/pi4 * ( d1_phi(a) * d1_phi(b) +  hh(a,b) * ( 0.5* kphi_sf*kphi_sf - tr_dphi_dphi))
             end if 
          end do
       end do
       ...
       !--- Build trace of T_{ab} as T=S-rho
       src_trT =  ww*ww*srcS_ww2 - srcE 


       !------------ Correct source terms ---------
       rhs_trk    = rhs_trk    + pi4  * alph * (srcE + ww*ww * srcS_ww2)/Bphi
       rhs_aa     = rhs_aa     - pi8  * alph * ww*ww * srcSijTF/Bphi
       rhs_gammat = rhs_gammat - pi16 * alph * srcji/Bphi
    ...
    if(CCTK_EQUALS(theory,"DEF") .OR. CCTK_EQUALS(theory,"BD")) then
        !----- In the eq for trK 
    F_phi   = 1.0/(2.0*(2*omega_phi + 3.0)) * (pi8 * src_trT  +  d_omega_phi * (BKphi*Bkphi - tr_dBphi_dBphi) )
    
    K_omega = alph * omega_phi * BKphi*Bkphi/(Bphi*Bphi)

    K_Bphi  = alph/Bphi * ( tr_cd2_Bphi - trk*BKphi - 3 * F_phi)
    
    rhs_trk = rhs_trk + K_omega + K_Bphi
            

        !---- In the eq for aa
    do a=1,3
        do b=1,3
            aa_omega(a,b) = -alph*omega_phi/( Bphi*Bphi ) *  ( ww*ww * d1_Bphi(a)*d1_Bphi(b)  -  hh(a,b) * tr_dBphi_dBphi/3 )
            aa_Bphi(a,b)  = alph/Bphi * ( aa(a,b)*BKphi -  cd2_Bphi(a,b)  +  hh(a,b) * tr_cd2_Bphi/3 ) 
        end do
    end do
    rhs_aa = rhs_aa + aa_omega + aa_Bphi

        !---- In the eq for gammat
    do a=1,3
        do b=1,3
           gammat_omega(a) = omega_phi/( Bphi*Bphi ) * BKphi * hu(a,b) * d1_Bphi(b)
           gammat_Bphi(a)  = ( hu(a,b) * d1_BKphi(b) - au(a,b) * d1_Bphi(b) - hu(a,b) * trk * d1_Bphi(b)/3 )/Bphi        
        end do
    end do    
    
    rhs_gammat = rhs_gammat + gammat_omega + gammat_Bphi
    end if

    !----------- Scalar field evolution----------
    rhs_vphi       = ad1_phi - alph * kphi_sf
    !------ DEF
    if(CCTK_EQUALS(theory,"DEF")) then
        rhs_vkphi      = ad1_kphi - alph * tr_cd2_phi - tr_dphi_dalph  -  alph * phi_sf * ( tr_dphi_dphi - kphi_sf*kphi_sf )  
        coupling_term  =  2 * pi * alph * B_DEF * src_trT * phi_sf / Bphi
    end if 
    !------- BD
    if(CCTK_EQUALS(theory,"BD")) then
        rhs_vkphi      = ad1_kphi - alph * tr_cd2_phi - tr_dphi_dalph  -  alph *  ( tr_dphi_dphi - kphi_sf*kphi_sf )  
        coupling_term  =  8 * pi * alph * C_BD * src_trT / Bphi
    end if 
    !----- decoupling
    if(CCTK_EQUALS(theory,"decoupling")) then 
        rhs_vphi       = ad1_phi - 2.0d0 * alph * kphi_sf
        rhs_vkphi      =  ad1_kphi +0.5*(- alph * ww*ww * tr_cd2_phi -tr_dphi_dalph + alph * ww *  tr_dww_dphi) !ad1_kphi - alph * tr_cd2_phi - tr_dphi_dalph  

        coupling_term  =  -2.0d0 * pi * alph * src_trT * (-B_DEF/2.0d0 *  phi_sf +0.01 )
     end if
     !------ GR
     if(CCTK_EQUALS(theory,"GR")) then
        rhs_vkphi       = 0.0  
        coupling_term   = 0.0
     end if 
   
    
     rhs_vkphi      = rhs_vkphi +  alph * trk * kphi_sf  + coupling_term
    ...
    rhs_phi(i,j,k)  = rhs_vphi
    rhs_kphi(i,j,k) = rhs_vkphi
  ...
  if (r(i,j,k)==0.0) THEN
          write(*,*) "---------------------------" 
        !-- Fundamental vars
         ! write(*,*) "The value of Bphi at r=0 is", Bphi
         ! write(*,*) "The value of Bkphi at r=0 is", BKphi
        !-- Omega funct
         ! write(*,*) "The value of omega at r=0 is", omega_phi
         ! write(*,*) "The value of d_omega at r=0 is", d_omega_phi
        !-- Advective derivatives
         ! write(*,*) "The value of ad1_alph at r=0 is", ad1_alph
         ! write(*,*) "The value of ad1_phi at r=0 is", ad1_phi
         ! write(*,*) "The value of ad1_kphi at r=0 is", ad1_kphi

          write(*,*) "The value of rhs_phi at r=0 is", rhs_vphi
          write(*,*) "The value of rhs_kphi at r=0 is", rhs_vkphi
          write(*,*) "The coupling term is ", coupling_term
         ! write(*,*) " gammat_omega(1)", gammat_omega(1)
         ! write(*,*) " gammat_Bphi(1) ", gammat_Bphi(1)
         ! write(*,*) "at r=0,  a(1,1)=",aa(1,1)

          !write(*,*) " aa_omega(1,1)", aa_omega(1,1) 
          !write(*,*) " aa_Bphi(1,1)", aa_Bphi(1,1)
          !write(*,*) "at r=0, K=", trk
          !write(*,*) " rhs_K", rhs_trk
          !write(*,*) " K_omega ", K_omega 
          !write(*,*) " K_Bphi", K_Bphi
          write(*,*) "tr_dphi_dalph  ", tr_dphi_dalph
          write(*,*) "tr_cd2phi  ", tr_cd2_phi
          write(*,*) "tr_dww_dphi  ", tr_dww_dphi
          write(*,*) "d2phi ", d2_phi
          write(*,*) "d1phi ", d1_phi
          write(*,*) "conformal factor ", ww 
          !write(*,*) "at r=0 gammat(1)=", gammat(1)

         ! write(*,*) "at r=0, T(0,0) is", Tab(4,4)
...
end subroutine JordanFBSSN_calc_bssn_rhs
!
!===========================================================================
!
subroutine JordanFBSSN_calc_bssn_rhs_bdry( CCTK_ARGUMENTS )
  ...
  if (CCTK_EQUALS(scalar_evolution_method, "JordanFBSSN")) then
          !-- In decoupling or GR phi(r->inf) = 0
     if(CCTK_EQUALS(theory,"decoupling") .OR. CCTK_EQUALS(theory,"GR")) then 
         ierr = NewRad_Apply(cctkGH, phi1, rhs_phi, phi1_0, one, n_phi1)
     else
         ierr = NewRad_Apply(cctkGH, phi1, rhs_phi, phi_at_inf, one, n_phi1)
     end if 
     ierr = NewRad_Apply(cctkGH, Kphi1, rhs_kphi, Kphi1_0, one, n_Kphi1)
  end if


end subroutine JordanFBSSN_calc_bssn_rhs_bdry
!
!===========================================================================
!
subroutine JordanFBSSN_calc_bssn_rhs_bdry_sph( CCTK_ARGUMENTS )
  ...
  if (CCTK_EQUALS(scalar_evolution_method, "JordanFBSSN")) then
     ...
     do k = cctk_lsh(3)-cctk_nghostzones(3)+1, cctk_lsh(3)
        do j = 1, cctk_lsh(2)
           do i = 1, cctk_lsh(1)
              rr        = r(i,j,k)
              dr_phi  = (phi1(i,j,k-2) - 4*phi1(i,j,k-1) + 3*phi1(i,j,k))*odr2
              rhs_phi(i,j,k)  = -dr_phi - (phi1(i,j,k) - 1.0) / rr
              dr_kphi  = (Kphi1(i,j,k-2) - 4*Kphi1(i,j,k-1) + 3*Kphi1(i,j,k))*odr2
              rhs_kphi(i,j,k)  = -dr_kphi - (Kphi1(i,j,k) - 1.0) / rr
           end do
        end do
     end do
  end if
...
end subroutine JordanFBSSN_calc_bssn_rhs_bdry_sph
