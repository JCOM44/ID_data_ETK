...
! --------------------------------------------
  ...
  if(n_pieces .gt. 0) then
     ...
     ! if improved piecewise polytropic
     if(improved_pp .eq. 1) then 
        allocate(hybrideos_Lambda(n_pieces))
     end if
     ...
     if( (n_pieces .eq. 2) .and. (hybrid_gamma(1) .eq. 0d0 ) )then
        ...
        do p = 1,n_pieces
           ...
           if(improved_pp .eq. 1) then 
               hybrideos_Lambda(p) = hybrid_Lambda(p)
           end if
        end do
     end if
     hybrideos_eps(1) = 0
...
!           hybrideos_eps(p+1) = hybrideos_eps(p)/hybrideos_rho(p) - 1.0d0  + &                  
!                hybrideos_k(p+1) * ( hybrideos_rho(p+1)**(hybrideos_gamma(p+1)-1.0d0) - &
!        hybrideos_rho(p)**(hybrideos_gamma(p+1)-1.0d0) )/(hybrideos_gamma(p+1)-1.0d0)

           write(*,*) " Eps ", hybrideos_eps(p+1)
        end do
     end if
  end if
